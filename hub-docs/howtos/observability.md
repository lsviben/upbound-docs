...
title: Observability
sidebar_position: 16
description: Collect Hub's own service metrics, install the hub-observability chart, and export them to your monitoring backend.
---
...
:::note[This is not the Metrics pipeline]
This page is about the health of the **Hub software itself**. It is separate from
...
- **Standard framework metrics** its runtime emits automatically — the
  ...
  full field list.
- **Hub instruments** (prefixed `hub_core_`) for the ingest and query paths and a
  few internal subsystems, catalogued below.
...
| `hub_core_ingest_stale_write_dropped_total` | counter | `control_plane`, `realm` | Upserts dropped because the incoming resource version was not newer than the stored one. Spikes indicate out-of-order delivery or a connector resync storm. |
...
managed-resource links) at ingest. Edges whose target has not been ingested yet
are parked in a backlog and promoted later by a background sweeper.
...
| `hub_core_edge_sweeper_cycle_capped_total` | counter | Sweeps that hit the per-cycle batch cap with a backlog remaining — the sweeper falling behind. |
...
pod — an OpenTelemetry Collector, Prometheus, and Grafana pre-wired — that
scrapes `hub-core:8085` into its own Prometheus and ships three preloaded
dashboards: **Hub Service Health**, **Hub Ingest**, and **Hub Query**.

The chart discovers Hub pods in its **own release namespace**, so install it in
the same namespace as Hub — `hub` in this guide:
...
The chart is **demo- and evaluation-grade**: one all-in-one pod with a single
PVC, no high availability. Use it to explore the metrics and dashboards, not to
back production alerting.
...
The three dashboards the chart ships are plain Grafana JSON built on PromQL. If
your backend serves the Prometheus query API, import them into your own Grafana
for the same views.
...
If you haven't yet, work through [Production hardening](production-overview.md) —
sizing, high availability, autoscaling, RBAC, and upgrades — then use this page to
watch the hardened install.
